...
module AXI_Slave (
  ...
  //--- Write helpers
  function automatic [31:0] data_wr_fixed(
    input [3:0]  wstrb,
    input [31:0] addr,
    input [31:0] wdata_i
  );
    ...
    begin
        for (i = 0; i < 4; i++)
            if (wstrb[i])
                memory[addr + i] = wdata_i[8*i +: 8];
        return addr;
    end
  endfunction

  function automatic [31:0] data_wr_incr(
    input [3:0]  wstrb,
    input [31:0] addr,
    input [31:0] wdata_i,
    input [2:0]  awsize
    ...
    integer i;
    begin
        $display("from DUT awburst=%0d address=%0d", awburst, addr);
          for(i=0; i<4; i++) if(wstrb[i]) memory[addr+i] = wdata_i[8*i +: 8];
      return addr + (1 << awsize);
    end
  endfunction

  function automatic [31:0] data_wr_wrap(
    input [3:0]  wstrb,
    input [31:0] addr,
    input [31:0] wdata_i,
    input [7:0]  boundary,
  ...
  function automatic [31:0] data_rd_incr(
    input [31:0] addr,
    input [2:0]  arsize
  );
    begin
  ...
  function automatic [31:0] data_rd_wrap(
    input [31:0] addr,
    input [7:0]  boundary,
    input [2:0]  arsize
  );
  ...
  always_comb begin
    ...
    case (wstate)
      ...
      wstart: if (wvalid) begin
        ...
        wready          = 1;
        wnext_state     = (wlast || (wlen_count_next == awlen)) ?
                          wlast_st : wreadys;
      end
    end
    ...
    wreadys: if (wvalid) begin
      ...
      nextaddr_next   = retaddr_next;
      wlen_count_next = wlen_count + 1;
      if (wlast || (wlen_count_next == awlen))
        wnext_state = wlast_st;
    end
